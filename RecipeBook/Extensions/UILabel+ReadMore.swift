...
//  RecipeBook
//
//  Created by David Farcas on 12.09.2023.
//
...
enum TrailingContent {
    ...
    var text: String {
        switch self {
        case .readmore:
            return LocalizedStrings.readMoreLabel.rawValue.localized()
        case .readless:
            return LocalizedStrings.readLessLabel.rawValue.localized()
        }
    }
}
...
extension UILabel {
    ...
    private var highlightColor: UIColor { return .tintColor }
    ...
    func appendReadmore(after text: String, trailingContent: TrailingContent) {
        ...
        while truncatedSentence.boundingRect(
            with: size,
            options: .usesLineFragmentOrigin,
            attributes: attributes,
            context: nil
        ).size.height >= fourlineHeight {
            if endIndex == 0 {
                break
            }
            endIndex -= 1
